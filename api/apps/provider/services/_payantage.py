...
This service is for Payantage Provider
...
class PayantageProviderService(BaseProvider):
    ...
    def send_request(self):
        ...
        try:
            ...
            logger.info(f"RAW PAYANTAGE REQUEST PAYLOAD:::{payload}")
            ...
            logger.info(f"RAW PAYANTAGE RESPONSE:::{resp}")
            json_resp = xmltodict.parse(resp)
            logger.info(f"JSON FORMATTED PAYANTAGE RESPONSE :::{json_resp}")
            ...
            logger.error(f"FAILED PAYANTAGE REQUEST TIMEOUT:: REASON::{e}")
            ...
            logger.error(f"FAILED PAYANTAGE REQUEST:, REASON::{e}", exc_info=True)
    ...
    def requery(self, transaction):
        """Requery transaction status from Payantage provider."""
        # TODO: Implement requery logic for Payantage
        return {
            "responseCode": "99",
